...
<div class="main-container">
    <form action="{{ route('nfts.filter') }}" method="get">
        @csrf
        <button class="btn btn-primary" type="submit" name="category_id" value="">All</button>
        @foreach($categories as $category)
            <button class="btn btn-primary" type="submit" name="category_id" value="{{ $category->id }}">{{ $category->name }}</button>
        @endforeach
    </form>
</div>
{{-- {{ $categories }} --}}
    ...
    <div class="row justify-content-center">
        <div class="container margin_60_35">
            <div class="main_title">
                <h2>Nft's world</h2>
                <p>Entering into possession of the nft </p>
            </div>
